...
if [[ "$MODEL_LABEL" == "copilot" ]]; then
    OUTPUT_DIR="./results/copilot/humaneval-jl-completions-baseline"
else
...
docker run --rm --platform linux/amd64 --network none -v ./$OUTPUT_DIR:/$OUTPUT_DIR:rw multipl-e-eval --dir /$OUTPUT_DIR --output-dir /$OUTPUT_DIR --recursive
